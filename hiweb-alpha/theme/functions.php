<?php
	...
	if( !function_exists( 'send_mail' ) ){
		...
		function send_mail( $to = '', $subject = '', $content = '', $from = 'noreply' ){ //TODO: не работает корректно
			...
			$headers = [ 'From: ' . get_bloginfo( 'name' ) . ' <' . $from . '@' . \hiweb\core\Paths\PathsFactory::root()->url()->get_domain() . '>' ];
			$headers[] = 'Reply-To: noreply@' . \hiweb\core\Paths\PathsFactory::root()->url()->get_domain() . '';
