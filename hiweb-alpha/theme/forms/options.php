<?php
	...
	add_field_text( 'theme-email-admin' )->label( 'Тема письма для администратора' )->description( $strtr_descriptions )->default_value( 'На сайте {site-name} была отправлена форма' )->Location( true );
...
С уважением, <a href="#{home-url}">{site-name}</a>' )->Location( true );
	...
	add_field_text( 'theme-email-client' )->label( 'Тема письма для заполнителя' )->description( $strtr_descriptions )->default_value( 'Вы заполнили форму на сайте {site-name}' )->Location( true );
...
С уважением, <a href="#{home-url}">{site-name}</a>' )->Location( true );
	...
	add_field_fontawesome( 'icon-process' )->default_value( 'fal fa-clock' )->label( 'Иконка процесса отправки' )->Location( true );
	add_field_fontawesome( 'icon-success' )->default_value( 'fal fa-comment-alt-check' )->label( 'Иконка удачной отправки сообщения' )->Location( true );
	add_field_fontawesome( 'icon-warn' )->default_value( 'fal fa-comment-exclamation' )->label( 'Иконка не верно заполненной формы' )->Location( true );
	add_field_fontawesome( 'icon-error' )->default_value( 'fal fa-comment-times' )->label( 'Иконка ошибки во время отправки' )->Location( true );
	add_field_textarea( 'text-process' )->default_value( 'Отправка сообщения...' )->label( 'Текст отправки формы' )->Location( true );
	add_field_textarea( 'text-success' )->default_value( 'Спасибо, сообщение было отправлено.' )->label( 'Текст удачной отправки формы' )->Location( true );
	add_field_textarea( 'text-warn' )->default_value( 'Сообщение не отправлено, не верно заполнена форма' )->label( 'Текст ошибки заполненной формы' )->Location( true );
	add_field_textarea( 'text-error' )->default_value( 'Ошибка во время отправки сообщения, попробуйте снова.' )->label( 'Текст ошибки в процессе отправки формы' )->Location( true );
	//
	...
	add_field_text( 'messages-rest-key' )->default_value( Strings::rand( 24, 1, 1, 1 ) )->label( 'Ключ, обязательный для сбора данных' )->description( 'Обязательно нажмите кнопку "сохранить", чтобы ключ зафиксировался.' )->Location( true );
	...
	add_field_text( 'messages-rest-limit' )->default_value( - 1 )->label( 'Сколько выводить сообщений в REST, -1 - без ограничений' )->Location( true );