		...
		class nav_menu{
			...
			public function the_list( $parent_id = 0, $depth = 0 ){
				if( $depth <= $this->depth && $this->has_subitems( $parent_id ) ){
					...
					$R = ob_get_clean();
					?>
					<ul class="<?= $depth == 0 ? implode( ' ', $this->ul_classes ) : '' ?> nav-level-<?= $depth ?>" <?= $depth > 0 ? 'style="visibility: hidden; position: absolute;"' : '' ?> data-items-count="<?= $items_count ?>" data-items-symbols-count="<?= $items_symbol_count ?>">
						<?php
							echo $R;
						?>
					</ul>
