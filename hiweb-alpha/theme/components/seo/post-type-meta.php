<?php
	...
	add_action( 'init', function(){
		...
		$post_type_enableNames = [];
		foreach( get_post_types() as $post_type ){
			if( seo::post_type_has_settings( $post_type ) ){
				$post_type_enableNames[] = $post_type;
				/** @var WP_Post_Type $post_type */
				$post_type = get_post_type_object( $post_type );
				if( seo::post_type_is_enable_customH1( $post_type ) ){
				
				}
				if( seo::post_type_is_enable_customLoopTitle( $post_type ) ){
				
				}
				if( get_option( 'hiweb-option-' . seo::$admin_menu_main . '-enable-' . $post_type->name ) ){
				
				}
			}
		}
		if( count( $post_type_enableNames ) > 0 ){
			add_field_text( 'seo-custom-h1' )->label( 'Индивидуальный заголовок H1' )->tooltip_help( 'Оставьте поле пустым для использования основного заголовка текущей страницы' )->location()->posts( $post_type_enableNames )->META_BOX()->title( 'SEO установки' )->context()->side();
			add_field_text( 'seo-custom-loop-title' )->label( 'Индивидуальный заголовок на архивной странице' )->tooltip_help( 'Оставьте поле пустым для использования основного заголовка текущей страницы' )->location( true );
			...
			add_field_separator( '' )->location( true );
			add_field_text( 'seo-meta-title' )->label( 'Заголовок (title)' )->location( true );
			add_field_textarea( 'seo-meta-description' )->label( 'Описание страницы (description)' )->location( true );
			add_field_text( 'seo-meta-keywords' )->label( 'Ключевые слова (keywords)' )->location( true );
			add_field_select( 'seo-meta-robots-mode' )->options( [
				'default' => '--выберите вариант--',
				'noindex' => 'noindex - Не индексировать текст страницы. Страница не будет участвовать в результатах поиска',
				'nofollow' => 'nofollow - Не переходить по ссылкам на странице	',
				...
				'archive' => 'archive - Отмена дерективы "noarchive"',
				'all' => 'all - Соответствует директивам index и follow — разрешено индексировать текст и ссылки на странице'
			] )->label( 'Режим индексации страницы и ссылок' )->tooltip_help( 'Данный режим добавить в шапку страницы таксономии мета тег <code>robots</code> внутри тега <code>head</code>.<br><a href="https://yandex.ru/support/webmaster/controlling-robot/meta-robots.html" target="_blank">Документация на Яндекс.Вебмастер</a>' )->location( true );
		}
	}, 30 );