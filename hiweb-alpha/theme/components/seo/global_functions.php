...
if ( !function_exists('get_the_term_archive_title')) {
     ...
     * @return mixed|string
     */
    function get_the_term_archive_title($wp_term, $default = '...', $esc_attr = true) {
        $title = $default;
        if ($wp_term instanceof WP_Term) {
            $title = get_field('seo-custom-loop-title', $wp_term, $default);
            if ($title == '') $title = $wp_term->name;
        }
        return apply_filters('get_the_term_archive_title', $esc_attr ? esc_attr($title) : $title, $wp_term, $default);
    }
}
