...
class form {
     ...
     * @version 1.1
     */
    public function get_inputs_options(): array {
        if ( !is_array($this->inputs_options)) {
            $this->inputs_options = [];
            $inputs = get_array(get_field('inputs', $this->wp_post))->rows();
            if ($inputs->have()) {
                while($inputs->have()) {
                    $this->inputs_options[] = $inputs->the();
                }
            }
        }
    ...
    public function get_inputs(): array {
        ...
        if ( !$included) {
            $included = true;
            get_file(__DIR__ . '/inputs')->include_files('php');
        }
