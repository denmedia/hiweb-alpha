...
add_action('admin_menu', function() {
    global $menu, $submenu;
    $submenu['edit.php?post_type=' . forms::$post_type_name][5][0] = get_fontawesome('fal fa-list-alt') . ' Формы';
    $submenu['edit.php?post_type=' . forms::$post_type_name][10][0] = get_fontawesome('fal fa-comment-alt-plus') . ' Создать форму';
});
///
...
$INPUTS->label('Поля ввода')->location()->posts(self::$post_type_name)->columnsManager()->name('Шорткоды')->callback(function($post_id) { hw_template_part('columns-shortcode', '', get_defined_vars()); });
//
