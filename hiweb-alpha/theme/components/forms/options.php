...
///Options
self::$options_object = add_admin_menu_page(self::$options_name, __('Options', 'hiweb-core-4'), 'edit.php?post_type=' . self::$post_type_name);
self::$options_object->icon_url('fas fa-cog');

add_field_tab(__('Main settings', 'hiweb-core-4'), __('Destination address of the recipient of messages from forms', 'hiweb-core-4'))->location()->options(self::$options_name);
add_field_text('email')->placeholder(get_bloginfo('admin_email'))->label(__('The mail address to which messages will be sent.', 'hiweb-core-4'))->description(sprintf(__('This will be the default address for receiving messages. If you leave this field empty, emails will be sent to the super-admin address <b>% s <a href="%s" data-tooltip="Change this address" title="Change this address"> <i class = "fas fa -pencil-alt "> </i> </a> </b> For each form, you can also set an individual address. You can also specify several addresses separated by commas or spaces, for example: <code> info@email.com admin@email.com </code>', 'hiweb-core-4'), get_bloginfo('admin_email'), get_admin_url(null, 'options-general.php#home-description')))->location()->options(self::$options_name);
...
add_field_tab(__('Privacy policy', 'hiweb-core-4'), __('Checkbox for form with information', 'hiweb-core-4'))->location()->options(self::$options_name);
...
///
add_field_separator('Шаблон письма для отправителя формы')->location(true);
