<?php
	...
	class hooks{
		...
		static function load_plugin_textdomain(){
			load_plugin_textdomain( 'hiweb-core-4', false, HW_MIGRATION_SIMPLE_DIR . '/languages' );
		}
		...
		static function plugin_action_links( $actions = [], $plugin_file = ''/*, $plugin_data = [], $context = null*/ ){
			if( $plugin_file == 'hiweb-migration-simple/hiweb-migration-simple.php' ){
				$actions['tool_panel'] = '<a href="' . self_admin_url( 'tools.php?page=hw_migration_simple' ) . '" class="tool_panel" aria-label="Открыть панель hiWeb Migration Simple">' . __( 'Migration Panel', 'hiweb-core-4' ) . '</a>';
			}
			return $actions;
		}
