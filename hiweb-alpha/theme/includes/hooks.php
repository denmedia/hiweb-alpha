<?php
	...
	add_action( 'wp_enqueue_scripts', function(){
		if( !\theme\includes\frontend::$use_wp_jquery_core && \hiweb\context::is_frontend_page() ){
			...
			$wp_scripts->registered['jquery-core']->deps = [\hiweb\js::get_handle($jquery_path)];
	...
	add_filter( 'script_loader_tag', function( $tag, $handle, $src ){
		if(!\theme\includes\frontend::$use_wp_jquery_core && \hiweb\context::is_frontend_page() && preg_match('|<script\b.*(\bdefer\b)[^>]+>|im', $tag) == 0) {
