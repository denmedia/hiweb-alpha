<?php
	...
	add_action( 'init', function(){
		$separator = add_field_separator( 'Заголовок архивной страницы' )->Location()->Options( theme\seo::$admin_menu_main );
		$has_archives = false;
		...
		foreach( get_post_types() as $post_type ){
			...
			if( $post_type->public && ( $post_type->_builtin || $post_type->publicly_queryable ) ){
				$has_public = true;
				add_field_checkbox( 'enable-' . $post_type->name )->label_checkbox( 'Включить поддержку настроек для "' . $post_type->label . '"' )->default_value( 'on' )->LOCATION( true )->FORM()->half();
				add_field_checkbox( 'enable-custom-h1-' . $post_type->name )->label_checkbox( 'Включить поддерджку индивидуального H1 для "' . $post_type->labels->name . '"' )->default_value( 'on' )->LOCATION( true )->FORM()->half();
				add_field_checkbox( 'enable-custom-loop-title-' . $post_type->name )->label_checkbox( 'Включить поддерджку индивидуального архивного заголовка "' . $post_type->labels->name . '"' )->default_value( 'on' )->LOCATION( true )->FORM()->half();
			}
		}
