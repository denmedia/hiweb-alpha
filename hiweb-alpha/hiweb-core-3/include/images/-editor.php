	...
	class editor extends path{
		...
		public function get_dimensions_by_resizeMod( $width, $height, $resize_mod = false ){
			...
			if( (int)$height > $this->height() )
				$height = $this->height();
			//Crop return
			if( $resize_mod === 0 || $resize_mod === true )
				return [ $width, $height ];
			//Support aspect and minMax
			if( $resize_mod < 0 )
				$dimensions = $this->get_dimensions_max( $width, $height ); else $dimensions = $this->get_dimensions_min( $width, $height );
			///
			return $dimensions;
		}
		...
		public function get_dimensions_by_dimensionsOrSizeName( $dimensionsOrSizeName = [ 150, 150 ], $resize_mod = 1 ){
			if( is_string( $dimensionsOrSizeName ) && trim( $dimensionsOrSizeName ) == '' )
				$dimensionsOrSizeName = [ $this->width(), $this->height() ];
			...
			if( $this->aspect() != 0 ){
				...
				if( $destination_file->is_classic_file_type() ){
					...
					if( extension_loaded( 'gd' ) ){
						...
						if( $this->get_image_mime_type() != 'image/jpg' ){
							///Use alpha chanel
						...
						$B = - 3;
						$temp_file = files::get( WP_CONTENT_DIR . '/hiweb-image-temp-' . strings::rand() . '.' . $destination_file->extension() );
						switch( $destination_file->get_image_mime_type() ){
							case 'image/jpg':
