<?php
	...
	class post_duplicator{
		...
		static function init(){
			...
			if( !$is_init ){
				$is_init = true;
				add_filter( 'page_row_actions', '\hiweb\tools\post_duplicator::_post_row_actions', 15, 2 );
				add_filter( 'post_row_actions', '\hiweb\tools\post_duplicator::_post_row_actions', 15, 2 );
				add_action( 'current_screen', function(){
					...
					if( get_current_screen()->base == 'edit' && get_current_screen()->post_type != '' && isset( $_GET['hiweb-post-duplicate'] ) ){
						if( wp_verify_nonce( $_GET['_wpnonce'], 'hiweb-post-duplicate' ) ){
							...
							wp_redirect( urls::get()->set_params( [ 'hiweb-post-duplicate' => null, '_wpnonce' => null, 'post_status' => 'draft' ] )->get_url( false ) );
						} else {
		...
		static function _post_row_actions( $actions, $post ){
			if( $post instanceof \WP_Post ){
				...
				$actions['hiweb-theme-post-duplicator'] = '<a data-hiweb-duplicate="' . $post->ID . '" href="' . get_url()->set_params( [ 'hiweb-post-duplicate' => $post->ID, '_wpnonce' => wp_create_nonce( 'hiweb-post-duplicate' ) ] )->get_url() . '">Дублировать <b>' . $wp_post_type->labels->name_admin_bar . '</b></a>';
			}
