<?php
	...
	class slide{
		...
		private function get_slide_class(){
			$classes = [ 'slide' ];
			if( $this->is_video )
				$classes[] = 'video'; elseif( $this->is_image )
				$classes[] = 'image';
			if( $this->darken )
				$classes[] = 'darken';
		...
		private function the_video(){
			...
			<video id="<?= $id_rand ?>" <?= wp_is_mobile() ? 'controls' : '' ?> width="100%" height="auto" preload="auto" playsinline <?= get_field( 'slider-mute' ) ? 'muted' : '' ?> <?= $poster ?> <?= $this->video_muted ? 'muted' : '' ?> >
				<?php if( $this->video_mp4 instanceof \hiweb\files\file && $this->video_mp4->is_readable() ){
					?>
					<source src="<?= $this->video_mp4->url ?>" type="video/mp4">
					<?php
				} ?>
				<?php if( $this->video_webm instanceof \hiweb\files\file && $this->video_webm->is_readable() ){
					?>
					<source src="<?= $this->video_webm->url ?>" type="video/webm">
					<?php
				} ?>
				<?php
					if( $this->video_ogv instanceof \hiweb\files\file && $this->video_ogv->is_readable() ){
						?>
