<?php
	...
	use hiweb_theme\widgets\forms\mailchimp;
	...
	class forms{
		...
		static protected $options_object_mailchimp = 'hiweb-forms-mailchimp';
		...
		static function init_mailchimp(){
			require_once __DIR__.'/forms/-options-mailchimp.php';
		}
		...
		static function mailchimp(){
			if(!self::$mailchimp instanceof mailchimp) {
				self::$mailchimp = new mailchimp( get_field('api-key',self::$options_name_mailchimp) );
			}
		...
		static function get_input_classes(){
			if( !is_array( self::$input_classes ) ){
				self::$input_classes = [];
				foreach( \hiweb\file( __DIR__ . '/forms/inputs' )->get_sub_files( [ 'php' ] ) as $path => $class_file ){
