<?php
	...
	class cache_item{
		 ...
		 * @return bool
		 */
		public function is_allowed_for_use( $return_error_no = false ){
			if( $return_error_no ){
				if( !options::is_enable() ) return - 1;
				if( !options::is_allow_url( tools::get_request_uri() ) ) return - 2;
				if( !$this->get_left_time() > 0 ) return - 3;
				if( isset( $_GET['nocache'] ) || isset( $_POST['nocache'] ) ) return - 4;
				return true;
			}
		 ...
		 * @return bool
		 */
		public function is_allowed_for_create( $ignore_expires = false ){
		...
		public function make( $ignore_expires = false ){
			if( !$this->is_allowed_for_create( $ignore_expires ) ) return - 1;
