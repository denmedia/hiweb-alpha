		...
		if( !is_admin() ){
			if( is_single() || is_page() ){
				...
				foreach( $langs as $lang_post ){
					\hiweb_theme\head::add_html_addition( '<link rel="alternate" hreflang="' . ( $lang_post->is_default() ? 'x-default' : $lang_post->get_lang_id() ) . '" href="' . get_permalink( $lang_post->ID ) . '" />' );
				}
			}
			if( is_archive() ){
				...
				foreach( $langs as $lang_term ){
					if( $lang_term instanceof WP_Term ){
						\hiweb_theme\head::add_html_addition( '<link rel="alternate" hreflang="' . ( $lang_term->is_default() ? 'x-default' : $lang_term->get_lang_id() ) . '" href="' . get_term_link( $lang_term->term_id ) . '" />' );
					}
				}
			}
		}
