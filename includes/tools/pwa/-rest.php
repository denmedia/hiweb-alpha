	...
	add_action( 'rest_api_init', function(){
		...
		register_rest_route( 'hiweb-theme', 'pwa/manifest', [
			...
			'callback' => function(){
				...
				if( $theme_color != '' ) $manifest->push( 'theme_color', $theme_color );
				if( $background_color != '' ) $manifest->push( 'background_color', $background_color );
				...
				if( $icon->is_attachment_exists() ){
					foreach( [ 192, 512 ] as $width ){
						if( $width > $icon->width() || $width > $icon->height() ) continue;
						$icon_src = $icon->get_file([$width, $width], 0, ['png']);
						$icons[] = [
					...
					if( $icon_splash->is_attachment_exists() ){
						if( $icon_splash->width() >= 512 && $icon_splash->height() >= 512 ){
							$icon_splash_size = $icon_splash->get_file([512,512], 0, ['png']);
							$icons[] = [
