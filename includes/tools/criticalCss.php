<?php
	...
	use hiweb_theme\tools\criticalCss\styles;
	use hiweb_theme\tools\criticalCss\template;
	...
	class criticalCss{
		...
		static function init(){
			if( !self::$init ){
				self::$init = true;
				require_once __DIR__ . '/criticalCss/-template.php';
				require_once __DIR__ . '/criticalCss/-styles.php';
				require_once __DIR__ . '/criticalCss/-hooks.php';
				///
				includes::jquery();
				///
				//cache dir
		...
		static function hooks_wp_head(){
			if( criticalCss::get_styles()->is_cache_exists() ){
				...
				pagesCache::force_stop_make_cache( 'critical css is created process...' );
				?>
		...
		static function get_current_theme_template_id( $return_md5hash = true ){
			...
			return md5( self::$current_theme_template_file );
		}
		...
		static function hook_shutdown(){
			if( context::is_frontend_page() && self::get_current_theme_template_id() != '' ){
				...
				if( $current_template->is_cache_exists() ){
					...
					criticalCss::get_template()->set_files( $R );
				}
			}
		}
