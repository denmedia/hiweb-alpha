	...
	class crumb{
		...
		public function init(){
			...
			elseif( $this->queried_object instanceof \WP_Post ) {
				...
				if( $this->queried_object->post_type == 'page' ){
					...
					if( !$this->parent_object->has_archive ){

						///fins in nav menus
						console_info(wp_get_nav_menus());

						///Collection test names
						$find_names = [ $this->parent_object->name, \theme\cyr3lat::ctl_sanitize_title( $this->parent_object->label ), \theme\cyr3lat::ctl_sanitize_title( $this->parent_object->labels->all_items ), $this->parent_object->label, $this->parent_object->labels->all_items ];
						$find_names = array_unique( $find_names );
						///find in pages
						foreach( $find_names as $find_name ){
							...
							if( count( $posts ) > 0 ){
								$this->parent_object = $posts[0];
								return;
							} else {
								$page = get_page_by_title( $find_name );
								if( $page instanceof \WP_Post ){
									$this->parent_object = $page;
									return;
								}
							}
						}
		...
		public function get_parent_crumb(){
			...
			if( $parent_object === false )
				return false; else return new crumb( $parent_object );
		}
