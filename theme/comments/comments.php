	...
	add_action( 'rest_api_init', function(){
		...
		register_rest_route( 'zorbasmedia', 'add-comment', [
			...
			'callback' => function(){
				...
				$post_id = \hiweb\urls::request( 'post_id' );
				$post = get_post( $post_id );
				if( !$post instanceof WP_Post ){
					return [ 'success' => false, 'message' => 'Не верно передан post_id' ];
				}
				$name = \hiweb\urls::request( 'name' );
				$text = \hiweb\urls::request( 'text' );
				...
				$comment_parent = intval( \hiweb\urls::request( 'comment_parent' ) );
