<?php
	...
	add_action( 'wp_loaded', function(){
		...
		foreach( languages::get_post_types( true ) as $post_type ){
			...
			add_filter( 'views_edit-' . $post_type, function( $views ){
				foreach( languages::get_languages() as $language ){
					$views[ 'hiweb-language-lang-id-' . $language->get_id() ] = '<a href="' . \hiweb\path::query( null, [ 'lang' => $language->get_id() ] ) . '" ' . ( \hiweb\path::request( 'lang' ) == $language->get_id() ? 'class="current"' : '' ) . ' aria-current="page">
			...
			add_filter( "manage_{$post_type}_posts_columns", function( $posts_columns ){
				$posts_columns = \hiweb\arrays::push( $posts_columns, 'Локализация', 4, languages::$post_columns_id );
	...
	add_action( 'current_screen', function(){
		console_info( get_term($_GET['tag_ID'], $_GET['taxonomy']) );
...
//	foreach( [ 'post', 'page' ] as $post_type_name ){
//
//		foreach( languages::get_languages() as $language ){
//			if( $language->is_default() ) continue;
//			add_field_text( $language->get_field_id( 'post_title' ) )->placeholder( 'Заголовок страницы (' . $language->get_name() . ')' )->label( 'Заголовок страницы (' . $language->get_name() . ')' )->LOCATION()->POST_TYPES( $post_type_name )->POSITION()->edit_form_after_title();
//		}
//
//		foreach( languages::get_languages() as $language ){
//			if( $language->is_default() ) continue;
//			add_field_content( $language->get_field_id( 'post_content' ) )->label( 'Содержимое страницы (' . $language->get_name() . ')' )->LOCATION()->POST_TYPES( $post_type_name )->POSITION()->edit_form_after_editor();
//		}
//	}
//
//	foreach( [ 'category', 'post_tag' ] as $taxonomy_name ){
//		foreach( languages::get_languages() as $language ){
//			if( $language->is_default() ) continue;
//			add_field_text( $language->get_field_id( 'name' ) )->label( 'Заголовок (' . $language->get_name() . ')' )->LOCATION()->TAXONOMIES( $taxonomy_name );
//		}
//	}
